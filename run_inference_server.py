...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--prompt",
        type=str,
        help="Optional system prompt. If omitted with --interactive, you can compose one on the fly.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="FLAG",
        help="Show contextual help for a specific flag (e.g. --explain ctx-size)",
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
        help="Open an interactive prompt composer when no --prompt is provided",
    )
    return parser
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not args.model:
        parser.error("--model is required for server runs")
    ...
    prompt = args.prompt
    if args.interactive and not prompt:
        try:
